# Codex Deck — Project Context for Codex

> This file is an internal context document that Codex reads when assisting on this repo.
...
## What This Is
A Tauri 2 desktop app (macOS) that manages multiple Codex sessions from one place.
The user (Dhiyanesh) runs 5+ Codex sessions simultaneously across Cursor, VS Code, and terminals.
Codex Deck gives a single dashboard to see all sessions, spawn new ones, and (eventually) approve permissions centrally.
...
process/pool.rs     — ProcessPool: spawns `Codex -p --stream-json`, manages stdin/stdout/stderr per session
process/events.rs   — NormalizedEvent enum + normalize_event() parser for Codex NDJSON protocol
session/discovery.rs — Scans ~/.Codex/sessions/*.json + ~/.Codex/projects/*/*.jsonl for metadata
...
## Codex NDJSON Protocol
Sessions are spawned with:
```
Codex -p --input-format stream-json --output-format stream-json --verbose --include-partial-messages
```
...
- **Phase 3 (Permission Hub):** Not started — HTTP permission server, hook injection into ~/.Codex/settings.json
...
Hub-spawned Codex sessions start but never stream responses. The process spawns successfully (PID exists) but no stdout NDJSON is received.
...
- We implemented `build_spawn_env()` in `pool.rs` which:
  ...
  3. Builds PATH from: `~/.Codex/local` + `/opt/homebrew/bin` + `/usr/local/bin` + login shell PATH + current PATH
- Stderr is forwarded to frontend as error events (but no errors are visible either)
- The Codex binary IS found and spawned (PID returned successfully)
...
### clui-cc (Electron Codex Desktop) Analysis
...
1. **stdin flush missing on initial prompt** — We `write_all` but don't `flush()` the initial prompt (we DO flush on follow-ups). Codex may be waiting for input.
...
Codex hot-reloads `~/.Codex/settings.json` with a file watcher. Hooks added there apply to ALL running sessions immediately. This means the Permission Hub can intercept permissions from ALL Codex sessions (Cursor, VS Code, terminal) — not just hub-spawned ones.

## User Context
- Dhiyanesh G, AI Engineer at DevRev
- Uses Codex via AWS Bedrock (not direct Anthropic API)
- Runs macOS, zsh shell
- Prefers concise communication, working code over lengthy explanations
- Building this for personal productivity across multiple simultaneous Codex sessions
